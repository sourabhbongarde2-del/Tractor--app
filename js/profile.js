...
import{addDoc,updateDoc,doc,db,col,uid,today,toast,lock,unlock,adBannerHTML,gProf,setProf,friendlyErr,LANG,CU,ONB,refreshOnboardState,nav,ensureQRCode}from'./core.js';
...
export async function pgProf(area){
  ...
  if(pr.upiId){
    ensureQRCode().then(()=>{
      ...
      const el=document.getElementById('pQR');
      if(el)el.innerHTML='<p style="font-size:.72rem;color:var(--tx3)">QR लोड होत नाही — इंटरनेट तपासा</p>';
    });
  }
}
