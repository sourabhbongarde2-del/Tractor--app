...
// ---- Lazy-load heavy third-party libs only when actually needed ----
// jsPDF (~350KB) + html2canvas (~150KB) + QRCode (~10KB) sirf Invoice/PDF/QR feature
// use hone par load hote hain - baaki sabhi sessions (jo sirf dashboard/khata/work-entry
// dekhte hain) is bandwidth ko save karte hain. Rural/patchy-internet audience ke liye
// yeh khaas zaroori hai - pehle yeh 3 scripts har page load pe <head> me render-blocking
// tarike se load hote the, chahe use ho ya na ho.
const _scriptCache={};
function loadScript(src){
  ...
  _scriptCache[src]=new Promise((resolve,reject)=>{
    ...
    s.src=src;
    s.onload=resolve;
    s.onerror=()=>{delete _scriptCache[src];reject(new Error('Script load failed: '+src));};
...
export async function ensurePdfLibs(){
  ...
  if(!window.jspdf)jobs.push(loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js'));
...
// ---- Announcement banner (admin can broadcast a message to all users) ----
// Har announcement ke saath ek 'updatedAt' version-stamp hota hai - dismiss karne pe
// SIRF wahi version localStorage me suppress hota hai, isliye admin jab NAYA message
// daalta hai (naya updatedAt), woh sabko phir se dikhega chahe purana dismiss kiya ho.
async function loadAnnouncement(){
  const el=document.getElementById('announceBanner');
  if(!el)return;
  try{
    const snap=await getDoc(doc(db,'announcements','current'));
    if(!snap.exists()){el.classList.add('h');return;}
    const a=snap.data();
    if(!a.active||!a.text){el.classList.add('h');el.innerHTML='';return;}
    const ver=a.updatedAt||'';
    if(localStorage.getItem('tw_announce_dismissed')===ver){el.classList.add('h');return;}
    el.innerHTML=`📢 ${a.text} <span style="cursor:pointer;font-weight:800;margin-left:10px" onclick="window._dismissAnnounce('${ver}')">✕</span>`;
    el.classList.remove('h');
  }catch(e){el.classList.add('h');}
}
window._dismissAnnounce=function(ver){
  localStorage.setItem('tw_announce_dismissed',ver);
  const el=document.getElementById('announceBanner');
...
function showLockScreen(){
  ...
  if(st&&st.mode==='disabled'){
    ...
    const price=window._globalSettings?.monthlyPrice||300;
    document.getElementById('lockMsg').textContent=`तुमचा 7-दिवसांचा मोफत ट्रायल संपला आहे. कृपया ₹${price}/महिना पेमेंट करून सुरू ठेवा.`;
  }
}
